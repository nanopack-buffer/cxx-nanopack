...
void SwiftInt32Generator::generate_field_declaration(CodeOutput &output,
...
void SwiftInt32Generator::generate_write_code(CodeOutput &output,
											 NanoPack::DataType *type,
											 const std::string &var_name) {
	output.stream() << "buf.append(int: " << var_name << ".littleEndian)" << std::endl;
}
...
void SwiftInt32Generator::generate_write_code(CodeOutput &output,
	...
	// clang-format off
	output.stream()
	<< "buf.write(size: 4, forField: " << field.field_number << ")" << std::endl;
	// clang-format on
